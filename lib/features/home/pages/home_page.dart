...
import 'package:myapp/features/deep_linking/services/dynamic_link_examples.dart';
...
class HomePage extends StatelessWidget {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () =>
                  context.push('${AppConstants.detailsRoute}/sample_123'),
              child: const Text('Go to Details (Push)'),
            ),
            const SizedBox(height: AppConstants.smallPadding),
            ElevatedButton(
              onPressed: () => context.go(
                '${AppConstants.reportRouteWithId('sample_report_123')}?source=homepage&data=abc',
              ),
              child: const Text('Go to Report (Replace)'),
            ),
            const SizedBox(height: AppConstants.smallPadding),
            ElevatedButton(
              onPressed: () => context.go(
                '${AppConstants.deepLinkRoute}?source=homepage&data=abc',
              ),
  ...
  Future<void> _generateQuickShare(BuildContext context) async {
    final dynamicLink = DynamicLinkGenerator.generateCustomLink(
      path:
          '${AppConstants.detailsRoute}/quick_share_${DateTime.now().millisecondsSinceEpoch}',
      queryParams: {
